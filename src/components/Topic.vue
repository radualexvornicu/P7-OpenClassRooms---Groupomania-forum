<template>
  <div class="jumbotron jumbotron-fluid d-flex flex-column p-1">
        ...
        <div class="card-header">
          <div id="v-switch main" v-switch="role">
            <div id="v-case 1" v-case="'ROLE_ADMIN'">
              <router-link
                :to="'/admin'"
                class="btn btn-warning rounded-circle m-1 pl-1 pr-1 pt-0 pb-0"
                >X</router-link
              >
            </div>
          ...
          <div class="d-flex justify-content-between align-items-center">
            ...
            <cite>
              <p class="text-sm-right">
                {{ topic.updatedAt | moment("from", "now") }}
              </p>
            </cite>
          </div>
          <div
          ...
          >
            <div>
              <h6>
                By:<router-link :to="'/profile/' + topic.userId">
                  {{ userNameTopic }}
                </router-link>
              </h6>
            </div>
      ...
      <div class="">
        <div class="mb-3">
          ...
          <select v-model="pageSizePost" @change="PageSizeChangePost($event)">
            <option
              v-for="sizePost in pageSizesPost"
              :key="sizePost"
              :value="sizePost"
            >
              {{ sizePost }}
    ...
    <ul class="col-md-9">
      <li
        class="list-group-item list-group-item-warning mb-2 rounded"
        v-for="(post, indexPost) in posts"
        :key="indexPost"
        @click="
          validateCheck();
          setActivePost(posts, indexPost);
          getUserNamePost(post.userId);
        "
      >
        <div class="d-flex justify-content-between align-items-center">
        ...
        <div class="" v-if="indexPost == currentIndexPost">
          <h6>
            By:
            <router-link
              :to="'/profile/' + currentPost[currentIndexPost].userId"
            >
              {{ userNamePost }}
            </router-link>
          </h6>
          <div id="v-switch main" v-switch="role">
            <div id="v-case 1" v-case="'ROLE_ADMIN'">
              <router-link
                :to="'/post/edit/' + currentPost[currentIndexPost].id"
              ...
              <button
                ...
                @click="
                  validateCheck();
                  removePost(topic.id, currentPost[currentIndexPost].id);
                "
              >
                Delete
              </button>
            ...
            <div v-default>
              <div v-if="!sameUserPost()">
                <button
                  type="button"
                  class="btn btn-success m-1 p-0"
                  @click="
                    validateCheck();
                    replyPost(currentPost[currentIndexPost].postcontent);
                  "
                >
                  Reply
                </button>
